...
async function fetchProfile(userId: string): Promise<Profile> {
  ...
  return {
    ...
    status: (data.status as string ?? 'active') as 'active' | 'deactivated' | 'blocked',
...
export async function login(input: LoginInput): Promise<AuthResponse> {
  ...
  if (profile.status === 'deactivated') {
    throw new AppError(403, 'Forbidden', 'Your account is deactivated. Contact support to reactivate.');
  }
